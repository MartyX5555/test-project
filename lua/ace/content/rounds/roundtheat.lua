...
ACE.AmmoBlacklist.THEAT = { "MG", "HMG", "RAC", "SL", "ECM", "ATR" , "AC" , "SA" , "GL","AAM","SAM", "FGL"}

local Round = {}

Round.type  = "Ammo" --Tells the spawn menu what entity to spawn
Round.name  = "[THEAT] - " .. ACETranslation.THEAT[1] --Human readable name
Round.model = "models/munitions/round_100mm_shot.mdl" --Shell flight model
Round.desc  = ACETranslation.THEAT[2]
...
function Round.convert( _, PlayerData )
	...
	Data.SlugMV = 1.15 * (Data.FillerMass / 2 * ACE.HEPower * (1 - Data.HEAllocation) * math.sin(math.rad(10 + GUIData.ConeAng) / 2) / Data.SlugMass) ^ ACE.HEATMVScaleTan --keep fillermass/2 so that penetrator stays the same
	Data.SlugMass = Data.SlugMass * 4 ^ 2
	Data.SlugMV = Data.SlugMV / 4

	Data.SlugMV2 = 1.15 * (Data.FillerMass / 2 * ACE.HEPower * Data.HEAllocation * math.sin(math.rad(10 + GUIData.ConeAng2) / 2) / Data.SlugMass2) ^ ACE.HEATMVScaleTan --keep fillermass/2 so that penetrator stays the same
...
function Round.getDisplayData(Data)
	...
	local SlugEnergy2	= ACE.Kinetic( Data.SlugMV2 * 39.37 , Data.SlugMass2, 999999 )
...
function Round.detonate( _, Bullet, HitPos, HitNormal )
	...
	ACE.HE( HitPos - Bullet.Flight:GetNormalized() * 3, HitNormal, Bullet.BoomFillerMass * (1-Bullet.HEAllocation), Bullet.CasingMass, Bullet.Owner, nil, Bullet.Gun )
	...
	ACE.HE( HitPos - Bullet.Flight:GetNormalized() * 3, HitNormal, Bullet.BoomFillerMass * Bullet.HEAllocation, Bullet.CasingMass, Bullet.Owner, nil, Bullet.Gun )
