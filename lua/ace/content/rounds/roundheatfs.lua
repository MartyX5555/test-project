...
ACE.AmmoBlacklist.HEATFS =  { "AC", "SA","C","MG", "AL","HMG" ,"RAC", "SC","ATR" , "MO" , "RM", "SL", "GL", "HW", "SC", "BOMB" , "GBU", "ASM", "AAM", "SAM", "UAR", "POD", "FFAR", "ATGM", "ARTY", "ECM", "FGL"}


local Round = {}

Round.type = "Ammo" --Tells the spawn menu what entity to spawn
Round.name = "[HEAT-FS] - " .. ACETranslation.ShellHEATFS[1] --Human readable name
Round.model = "models/munitions/round_100mm_mortar_shot.mdl" --Shell flight model
Round.desc = ACETranslation.ShellHEATFS[2]
...
function Round.convert( _, PlayerData )
	...
	PlayerData.ConeAng = math.max(PlayerData.ConeAng or 0, 0)
	...
	ConeLength, ConeArea, AirVol = Round.ConeCalc( PlayerData.ConeAng, Data.Caliber / 2, Data.ProjLength )
	...
	Data.ProjMass = math.max(GUIData.ProjVolume - PlayerData.FillerVol, 0) * 7.9 / 1000 + math.min(PlayerData.FillerVol, GUIData.ProjVolume) * ACE.HEDensity / 1000 + ConeArea * ConeThick * 7.9 / 1000
	...
	--local MaxLength = 0
	--local MaxRadius = 0
	MaxVol, MaxLength, MaxRadius = ACE.RoundShellCapacity( Energy.Momentum, Data.FrArea, Data.Caliber, Data.ProjLength )
	...
	--print('Current filler: ' .. PlayerData.FillerVol)

	GUIData.MinFillerVol = 0
	GUIData.MaxFillerVol = math.max(MaxVol -  AirVol - ConeVol,GUIData.MinFillerVol) * 0.95
	GUIData.FillerVol = math.Clamp(PlayerData.FillerVol,GUIData.MinFillerVol,GUIData.MaxFillerVol)

	--print('After filler: ' .. PlayerData.FillerVol)
	...
	Data.SlugMV = 2.2 * (Data.FillerMass / 2 * ACE.HEPower * math.sin(math.rad(10 + GUIData.ConeAng) / 2) / Data.SlugMass) ^ ACE.HEATMVScale --keep fillermass/2 so that penetrator stays the same --1.3
...
function Round.getDisplayData(Data)
	...
	local SlugEnergy = ACE.Kinetic(Data.SlugMV * 39.37, Data.SlugMass, 999999)
...
ACE.HE( HitPos - Bullet.Flight:GetNormalized() * 3, HitNormal, Bullet.BoomFillerMass, Bullet.CasingMass, Bullet.Owner, nil, Bullet.Gun )
...
function Round.propimpact( Index, Bullet, Target, HitNormal, HitPos, Bone )
	...
	if ACE.Check( Target ) then
		...
		if Bullet.Detonated then
			...
			local Energy = ACE.Kinetic( Speed , Bullet.ProjMass, 999999 )
...
function Round.worldimpact( Index, Bullet, HitPos, HitNormal )
	...
	local Energy = ACE.Kinetic( Bullet.Flight:Length() / ACE.VelScale, Bullet.ProjMass, 999999 )
...
function Round.guicreate( Panel, Table )
	...
	acemenupanel:AmmoSelect( ACE.AmmoBlacklist.HEATFS )
